...
Write-Host "Starting Deployment for Multimedia Edition (v11.0): $PROJECT_ID" -ForegroundColor Cyan
...
gcloud iam service-accounts create $SA_NAME --display-name="Morgenpost Gmail SA" --project $PROJECT_ID 2>$null
...
gcloud secrets create GMAIL_APP_PASSWORD --project $PROJECT_ID 2>$null

Write-Host "Building and Deploying..." -ForegroundColor Yellow
gcloud artifacts repositories create morgenpost-repo --repository-format=docker --location=$REGION --project $PROJECT_ID 2>$null
...
Write-Host "Done! Multimedia Edition Deployment successful." -ForegroundColor Green
